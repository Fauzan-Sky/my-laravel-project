...
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .sidebar-kantin .kantin-name {
            font-size: 14px;
            font-weight: 600;
            color: #fff;
        }

        .sidebar-kantin .kantin-status {
            display: inline-flex;
            align-items: center;
            gap: 5px;
            font-size: 11px;
        ...
        .sidebar-kantin .kantin-status::before {
            content: '';
        ...
        .stat-icon.red    { background: #fef0ee; }
...
<aside class="sidebar">
    ...
    <div class="sidebar-kantin">
        ...
        <div class="kantin-name">{{ auth('penjual')->user()->kantin->nama_kantinn ?? 'Kantin Saya' }}</div>
        <div class="kantin-status">Sedang Buka</div>
    </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $m->nama_menu }}</td>
                                <td>
                                    <span class="stok-value {{ $m->stok <= 5 ? 'stok-low' : 'stok-mid' }}">
                                        {{ $m->stok }}
                                    </span>
                                </td>
                                <td>
                                    <span class="badge {{ $m->stok <= 5 ? 'badge-pending' : 'badge-processing' }}">
                                        {{ $m->stok <= 5 ? 'Kritis' : 'Menipis' }}
                                    </span>
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="3">
                                    <div class="empty-state">
                                        <div class="empty-icon" style="color:#27ae60"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 15.01 9 12.01"/></svg></div>
                                        <p>Semua stok aman</p>
                                    </div>
                                </td>
                            </tr>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr data-status="{{ $p->status }}" class="{{ $p->status === 'pending' ? 'row-pending' : '' }}">
                            <td>
                                @if($p->status === 'pending')
                                    <span class="new-order-dot"></span>
                                @endif
                                <span class="antrean-badge">#{{ str_pad($p->nomor_antrean, 3, '0', STR_PAD_LEFT) }}</span>
                            </td>
                            <td>
                                <div style="font-weight:600; font-size:13px">{{ $p->user->nama_lengkap ?? '-' }}</div>
                            </td>
                            ...
                            <td style="font-size:12px">
                                @if($p->slotWaktu)
                                    ...
                                    <div style="color:var(--muted)">
                                        {{ \Carbon\Carbon::parse($p->slotWaktu->jam_mulai)->format('H:i') }} –
                                        {{ \Carbon\Carbon::parse($p->slotWaktu->jam_selesai)->format('H:i') }}
                                    </div>
                                @else
                    ...
                    <tbody>
                        ...
                        <tr>
                                ...
                                <div style="display:flex; flex-direction:column; align-items:center; gap:8px;">
                                    ...
                                    @else
                                        <div style="width:56px; height:56px; background:var(--teal-light); border-radius:10px; display:flex; align-items:center; justify-content:center;">
                                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--teal)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                                                <rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/>
                                                <polyline points="21 15 16 10 5 21"/>
                                            </svg>
                                        </div>
                                    @endif
                                    <form method="POST" action="{{ route('penjual.stok.uploadFoto', $menu->id) }}"
                                          enctype="multipart/form-data"
                                          style="display:flex; flex-direction:column; align-items:center; gap:4px;">
                                        @csrf
                                        <input type="file" name="foto" accept="image/*"
                                               style="display:none" id="foto-{{ $menu->id }}"
                                               onchange="this.form.submit()">
                                        <label for="foto-{{ $menu->id }}"
                                               style="font-size:11px; font-weight:600; color:var(--teal); cursor:pointer; padding:3px 8px; border:1px solid var(--teal); border-radius:6px; transition:all 0.2s;"
                            ...
                            <td>
                                ...
                                @if($menu->deskripsi)
                                    <div style="font-size:11px; color:var(--muted)">{{ Str::limit($menu->deskripsi, 40) }}</div>
                                @endif
                            </td>
                            <td>
                                <span class="badge badge-processing" style="text-transform:capitalize">
                                    {{ $menu->kategori ?? '-' }}
                                </span>
                            </td>
                            <td style="font-weight:600">Rp {{ number_format($menu->harga, 0, ',', '.') }}</td>
                            <td>
                                <span class="stok-value {{ $menu->stok <= 5 ? 'stok-low' : ($menu->stok <= 15 ? 'stok-mid' : 'stok-high') }}">
                                    {{ $menu->stok }}
                                </span>
                            </td>
                            <td>
                                <form method="POST" action="{{ route('penjual.stok.update', $menu->id) }}"
                                      style="display:flex; gap:6px; align-items:center">
                                    @csrf
                                    @method('PATCH')
                                    <input type="number" name="stok" class="stok-input"
                                           value="{{ $menu->stok }}" min="0" max="999">
                        ...
                        <tr>
                            <td colspan="6">
                                <div class="empty-state">
                                    <div class="empty-icon" style="color:#ccc"><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 2v7c0 1.1.9 2 2 2h4a2 2 0 0 0 2-2V2"/><path d="M7 2v20"/><path d="M21 15V2v0a5 5 0 0 0-5 5v6c0 1.1.9 2 2 2h3zm0 0v7"/></svg></div>
                                    <p>Belum ada menu. Tambahkan menu terlebih dahulu.</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
